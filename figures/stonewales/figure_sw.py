...
ax.plot([distances1[min1], distances1[max2]], [reaction1[min1], reaction1[min1]], '--k')
ax.plot([distances2[0],distances2[max3]], [reaction2[0], reaction2[0]], '--k')
ax.arrow(distances1[max1], reaction1[max1]/2, 0, reaction1[max1]/2-0.02, width=0.005, facecolor='k',
        length_includes_head=True, head_width=0.025, head_length = 0.1)
ax.arrow(distances1[max1], reaction1[max1]/2, 0, -reaction1[max1]/2+0.02, width=0.005, facecolor='k',
        length_includes_head=True, head_width=0.025, head_length = 0.1)
ax.arrow(distances1[max2], h1, 0, reaction1[max2]-h1-0.02, width=0.005, facecolor='k',
        length_includes_head=True, head_width=0.025, head_length = 0.1)
ax.arrow(distances1[max2], h1, 0, -(reaction1[max2]-h1-0.02), width=0.005, facecolor='k',
        length_includes_head=True, head_width=0.025, head_length = 0.1)
ax.arrow(distances2[max3], h2, 0, reaction2[max3]-h2-0.02 ,  width=0.005, facecolor='k',
        length_includes_head=True, head_width=0.025, head_length = 0.1)
ax.arrow(distances2[max3], h2, 0, -(reaction2[max3]-h2-0.02), width=0.005, facecolor='k',
        length_includes_head=True, head_width=0.025, head_length = 0.1)

ax.text(distances1[max1] - 0.5, reaction1[max1]/2.1, '{:4.2f} eV'.format(reaction1[max1]), fontsize = 20)
ax.text(distances1[max2] + 0.08, h1, '{:4.2f} eV'.format(reaction1[max2]-reaction1[min1]), fontsize = 20)
ax.text(distances2[max3] + 0.08, h2, '{:4.2f} eV'.format(reaction2[max3]-reaction2[0]), fontsize = 20)
...
ab2 = AnnotationBbox(imagebox2, [distances1[max1], reaction1[max1]+0.02],
                    xybox = [0.30 * distances2[-1], 6.5],
                    arrowprops=dict(arrowstyle = "->", linewidth=3))
ab3 = AnnotationBbox(imagebox3, [distances1[max2], reaction1[max2]+0.02],
                    xybox = [0.5 * distances2[-1], 6.5],
                    arrowprops=dict(arrowstyle = "->", linewidth=3))
ab4 = AnnotationBbox(imagebox4, [distances1[-1], reaction1[-1]-0.02],
                    xybox = [0.60 * distances2[-1], 1.4],
                    arrowprops=dict(arrowstyle = "->", linewidth=3))
ab5 = AnnotationBbox(imagebox5, [distances2[max3], reaction2[max3]-0.02],
                    xybox = [0.79 * distances2[-1],3.3],
                    arrowprops=dict(arrowstyle = "->", linewidth=3))
ab6 = AnnotationBbox(imagebox6, [distances2[-1], reaction2[-1]-0.02],
